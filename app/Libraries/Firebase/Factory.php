...
use Throwable;
use App\Libraries\Firebase\Auth\IdTokenVerifier;
use App\Libraries\Firebase\Auth\SessionCookieVerifier;
use Kreait\Firebase\ServiceAccount;
use App\Libraries\Firebase\Auth\ApiClient;
use App\Libraries\Firebase\Auth\Token\Verifier as LegacyIdTokenVerifier;
...
class Factory
{
    ...
    public function createAuth(): \Kreait\Firebase\Contract\Auth
    {
        $httpClient = $this->createApiClient([
            'base_uri' => 'https://www.googleapis.com/identitytoolkit/v3/relyingparty/',
        ]);

        $authApiClient = new ApiClient($httpClient, $this->tenantId);
        ...
        return new Auth($authApiClient, $httpClient, $customTokenGenerator, $idTokenVerifier, $sessionCookieVerifier, $signInHandler, $this->tenantId, $this->getProjectId());
    }
    ...
    public function createIdTokenVerifier()
    {
        if (!($projectId = $this->getProjectId())) {
    ...
    public function createSessionCookieVerifier()
    {
        if (!($projectId = $this->getProjectId())) {
            return new DisabledLegacyIdTokenVerifier(
                'ID Token Verification is disabled because no project ID was provided'
